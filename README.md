...
Configure mock-SAP behavior globally with `MOCK_SAP_MODE` and `MOCK_SAP_DELAY_MS` in `.env`. The dashboard’s Mock SAP panel can configure a persistent override for all `POST /api/orders`, including status, JSON body, `Retry-After`, and an optional capped delay. The override remains active until replaced or reset. Overrides can also be managed directly with authenticated `GET`, `PUT`, and `DELETE /api/admin/response` requests using `X-Mock-SAP-Admin-Token`. The local default token is `local-dashboard-token`; production Compose requires `MOCK_SAP_ADMIN_TOKEN`. Inspect request/response logs with `docker compose logs -f mock-sap`. Listener reconnect backoff, startup recovery of stale jobs, and dispatch timing telemetry belong to the order-sync service; this helper dashboard displays the persisted order state without duplicating that worker logic.
